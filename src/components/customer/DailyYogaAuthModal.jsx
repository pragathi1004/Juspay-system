...
export const DailyYogaAuthModal = ({ isOpen, onClose, initialStep = 'PHONE' }) => {
  ...
  const handleAlreadySubscribedClick = () => {
    setStep('ALREADY_SUBSCRIBED');
  };

  const handleExistingSubscriberLogin = (e) => {
    e?.preventDefault();
    if (!registeredPhone || registeredPhone.length < 10) {
      alert('Please enter a valid 10-digit registered number');
      return;
    }
    
    // Direct dashboard access for existing subscriber
    setUserFlow('EXISTING_MEMBER');
    const nameParts = (registeredName || 'PRAGATHI').trim().split(' ');
    const fName = nameParts[0] || 'PRAGATHI';
    const lName = nameParts.slice(1).join(' ') || 'M';

    setRegForm(prev => ({
      ...prev,
      firstName: fName,
      lastName: lName,
      name: registeredName || 'PRAGATHI',
  ...
  const handleSubscriberOtpLogin = () => {
    setPendingAuth({
      type: 'Subscriber Login OTP',
      extraDetails: { phone: registeredPhone, name: registeredName }
    });
    setTimer(30);
    setStep('OTP');
  };
    ...
    >
      ...
      >
        ...
        <div style={{ padding: '24px 28px 16px 28px', display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center' }}>
          ...
          {/* ========================================================================= */}
          {/* STEP 1: MAIN ENTRY - PHONE NUMBER INPUT (SCREEN 6) */}
          {/* ========================================================================= */}
          {step === 'PHONE' && (
            <>
              ...
              {/* 3 SOCIAL LOGIN BUTTONS ROW (GOOGLE, FACEBOOK, EMAIL) */}
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '12px', width: '100%', marginBottom: '14px' }}>
